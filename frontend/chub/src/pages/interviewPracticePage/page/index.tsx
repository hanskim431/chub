...
import {
    ...
    useSubmitAnswer,
    useCompletePractice,
} from "@/pages/interviewPracticePage/api/query";
import type { PracticeAnswer } from "@/pages/interviewPracticePage/api/requests";

export default function InterviewPracticePage() {
    const navigate = useNavigate();
    const { id } = useParams<{ id: string }>();
    const { mutate: startPractice, isPending: isStarting } = useStartPractice();
    const { data: sessionData, isLoading: isLoadingSession } =
        usePracticeSession(id ? Number(id) : undefined);
    const { mutate: submitAnswer, isPending: isSubmitting } =
        useSubmitAnswer();
    const { mutate: completePractice, isPending: isCompleting } =
        useCompletePractice();
            ...
            {},
            {
                onSuccess: (response) => {
                    if (response?.data?.data) {
                        navigate(
                            `/interviews/practice/${response.data.data.practiceId}`
                        );
                    }
                },
                onError: () => {
                    alert("면접 연습을 시작할 수 없습니다.");
                },
            }
        );
    };
    ...
    const handleComplete = () => {
        ...
        handleSaveAnswer();
        completePractice(Number(id), {
            onSuccess: () => {
                alert("면접 연습이 완료되었습니다.");
                navigate("/dashboard");
            },
            onError: () => {
                alert("면접 연습 완료에 실패했습니다.");
            },
        });
    };
    ...
    if (!id) {
        return (
            <div className="container mx-auto px-4 py-8 max-w-4xl">
                <Card className="p-8">
                    <h1 className="text-3xl font-bold mb-4">면접 연습</h1>
                    <p className="text-gray-600 mb-8">
                        면접 연습을 통해 실전 면접에 대비하세요. 다양한 질문에
                        답변하며 면접 실력을 향상시킬 수 있습니다.
                    </p>
                    <button
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            <div className="mb-6">
                ...
                <h1 className="text-3xl font-bold mb-2">면접 연습</h1>
