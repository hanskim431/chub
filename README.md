...
### 🔄 **Lazy Loading 기반 미읽음 추적으로 빠른 채팅 환경 구축**

실시간 채팅 시스템에서 읽음/안읽음 상태를 정확히 추적하는 것은 사용자 경험의 핵심입니다. 기존의 단순한 방식들은 매 요청마다 메시지를 재계산하거나, 메모리에만 저장해 성능 저하와 동시성 문제를 야기했습니다.

이를 해결하기 위해 **Lazy Loading 기반 미읽음 추적**를 도입했습니다. unreadCount, lastReadAt, countedAt 필드로 읽음 상태를 추적하고, 마지막 계산 시간을 체크해 불필요한 재계산을 방지합니다.

WebSocket 기반 실시간 읽음 영수증으로 상대방에게 즉시 알림을 전송하고, MongoDB 원자적 업데이트로 동시성 문제를 해결했습니다. 결과적으로 새 메시지가 없는 경우 countedAt 체크로 DB 쿼리를 스킵해 채팅방 목록 로딩 시간을 O(1)의 시간 복잡도로 개선했습니다.
