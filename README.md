...
| **김형진**  |- 면접관 목록 페이지 구현으로 RecruiterListPage, RecruiterListItem 컴포넌트 및 필터링/페이지네이션 제공<br>- Card, Pill, SkeletonLine, Pagination 등 재사용 가능한 UI 컴포넌트를 TDD 방식으로 개발<br>- Zustand 상태 관리를 통한 면접관 목록 필터링 및 API 연동<br>- WebSocket 기반 인터뷰룸 통합으로 실시간 채팅, ICE Candidate 관리, 메시지 송수신 구현<br>- 오디오 레코딩 및 STT 연동으로 질문/답변 음성 기반 입력 및 오디오 제어 UI 개발<br>- 완료된 면접 기록 페이지에서 질문-답변 쌍 표시 및 면접 결과 조회 기능 구현<br>- FloatingChat, ChatPanel 컴포넌트를 통한 실시간 채팅 및 WebSocket 구독/취소 관리<br>- React Testing Library를 통한 컴포넌트 단위 테스트 작성(TDD)<br>|
